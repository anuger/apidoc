...
function createDoc(options) {
  ...
  try {
    ...
    // start a http server
    if (options.staticServer || options.mockServer)
      startMockSever(api);
    app.log.info('Done.');
...
function createOutputFiles(api) {
  ...
  if (!app.options.simulate)
    fs.writeFileSync(app.options.dest + './api_data.js', 'define({ "api": ' + createMockApi(api.data) + ' });' + '\n');
...
function createMockApi(apiData) {
  apiData = JSON.parse(apiData);
  apiData.forEach(ele => {
    if (ele.success && ele.success.examples) {
      var response;
      try {
        response = JSON.parse(ele.success.examples[0].content);
      } catch (e) {
        app.log.warn(ele.url);
        throw e;
      }
      ele.success.examples[0].content = JSON.stringify(Mock.mock(response));
    }
  });
  return JSON.stringify(apiData);
}

function startMockSever(api) {
  
  // start static server
  if (app.options.staticServer) {
    server.use(express.static(app.options.dest));
  }
  
  // start mock server
  if (app.options.mockServer) {
    JSON.parse(api.data).forEach(ele => {
      server[ele.type](ele.url, function (req, res) {
        if (ele.success && ele.success.examples) {
          var response = JSON.parse(ele.success.examples[0].content);
          res.json(Mock.mock(response));
        } else {
          res.json({
            'state': 500,
            'message': 'apiSuccessExample is error'
          });
        }
      });
    });
  ...
  server.listen(app.options.port, () => {
    app.log.info("start server on " + app.options.port);
  });
}
